...
The repository SHALL provide an automated secret-scanning path for local development and CI. The default scanning workflow MUST detect newly introduced credential-like material in tracked changes before those changes are merged, using a reproducible repository-defined configuration.
...
The secret-scanning workflow SHALL support explicit exclusions and reviewed allowlists for known false-positive locations such as vendored dependencies, generated assets, CI-only placeholder values, development example files, and non-secret documentation or test examples. Exceptions MUST be narrow enough to avoid suppressing unrelated findings.
...
- **WHEN** a reviewed example, CI-only placeholder, or development fixture string matches a scanner rule without containing a real secret
