...
celery_data_dir = os.environ.get(
    "MAXKB_CELERY_DATA_DIR", os.path.join(PROJECT_DIR, "data", "celery_task")
)
if not os.path.exists(celery_data_dir) or not os.path.isdir(celery_data_dir):
    ...
    celery_data_parent = os.path.dirname(celery_data_dir)
    if os.path.isdir(celery_data_parent):
        os.chmod(celery_data_parent, 0o700)
