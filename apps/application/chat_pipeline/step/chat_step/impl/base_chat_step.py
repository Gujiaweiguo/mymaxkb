...
class BaseChatStep(IChatStep):
    ...
    def _handle_mcp_request(
        ...
        if skill_tool_ids and len(skill_tool_ids) > 0:
            ...
            for tool_id in skill_tool_ids:
                ...
                if tool.init_params is not None:
                    params = init_params_default_value | json.loads(
                        rsa_long_decrypt(tool.init_params)
                    )
                else:
                    params = init_params_default_value

                skill_file_items.append(
                    {"tool_id": str(tool.id), "file_id": tool.code, "params": params}
                )
    ...
    def get_stream_result(
        ...
        if chat_model is None:
            ...
            # 过滤tool_id
            all_tool_ids = list(
                set((mcp_tool_ids or []) + (tool_ids or []) + (skill_tool_ids or []))
            )
            authorized_set = set(
                filter_authorized_ids("tool", all_tool_ids, workspace_id)
            )
