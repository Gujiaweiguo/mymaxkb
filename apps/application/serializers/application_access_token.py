...
class AccessTokenEditSerializer(serializers.Serializer):
    ...
    access_num = serializers.IntegerField(
        required=False, max_value=10000000, min_value=0, label=_("Number of visits")
    )
    ...
    icon = serializers.FileField(required=False, allow_null=True, label=_("icon"))
...
class AccessTokenSerializer(serializers.Serializer):
    ...
    def edit(self, instance):
        ...
        if application is None:
            raise AppApiException(500, _("Application id does not exist"))
        if "icon" in instance and instance.get("icon") is not None:
            application.icon = FileSerializer(
                data={
                    "file": instance.get("icon"),
                    "source_id": str(application_id),
                    "source_type": FileSourceType.APPLICATION,
                    "meta": {"field": "icon"},
                }
            ).upload()
            application.save(update_fields=["icon", "update_time"])
    ...
    def one(self, with_valid=True):
        ...
        return {
            ...
            "icon": QuerySet(Application).filter(id=application_id).first().icon,
