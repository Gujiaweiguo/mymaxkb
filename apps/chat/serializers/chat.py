    ...
    class PostHandler(PostResponseHandler):
        def handler(
            ...
            **kwargs,
            ...
            merged_source = (
                {
                    **(chat_info.source if isinstance(chat_info.source, dict) else {}),
                    **(
                        existing_chat_record.source
                        if existing_chat_record is not None
                        and isinstance(existing_chat_record.source, dict)
                        else {}
                    ),
                }
                if isinstance(chat_info.source, dict)
                or (
                    existing_chat_record is not None
                    and isinstance(existing_chat_record.source, dict)
                )
                else chat_info.source
            )
            if existing_chat_record is not None:
                chat_record = existing_chat_record
                chat_record.problem_text = problem_text
                chat_record.answer_text = answer_text
                chat_record.details = manage.get_details()
                chat_record.message_tokens = manage.context["message_tokens"]
                chat_record.answer_tokens = manage.context["answer_tokens"]
                chat_record.answer_text_list = answer_list
                chat_record.run_time = manage.context["run_time"]
                chat_record.ip_address = chat_info.ip_address
                chat_record.source = merged_source
            else:
    ...
    def is_valid_chat_user(self):
        ...
        if (
            ...
            if chat_user_type == ChatUserType.CHAT_USER.value:
                is_auth = (
                    is_auth_chat_user(chat_user_id, application_id)
                    if is_auth_chat_user
                    else is_auth_application_chat_user(chat_user_id, application_id)
                )
                if not is_auth:
