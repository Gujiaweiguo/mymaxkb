...
from users.serializers.wecom_login import WecomIdentitySerializer


class WecomAuthenticationSerializer(serializers.Serializer):
    ...
    def auth(self):
        ...
        identity = WecomIdentitySerializer.fetch_identity(
            SettingType.CHAT_USER_PLATFORM_SOURCE,
            self.validated_data["code"],
        )
        username = WecomIdentitySerializer.resolve_username(identity)
        chat_user = QuerySet(ChatUser).filter(username=username).first()
        if chat_user is None:
            raise AppUnauthorizedFailed(
                500, _("WeCom user is not bound to an existing chat account")
            )
        ...
        return ChatUserToken(
            ...
            str(chat_user.id),
            ChatAuthentication("wecom", username=username),
        ).to_token()
