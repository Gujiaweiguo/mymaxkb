...
class DingtalkIdentitySerializer(serializers.Serializer):
    ...
    def get_provider_credentials(setting_type: int):
        ...
        provider = (
            meta.get("dingtalk") if isinstance(meta.get("dingtalk"), dict) else {}
        )
        config = (
            provider.get("config") if isinstance(provider.get("config"), dict) else {}
        )
        if not provider.get("is_valid"):
            raise AppApiException(500, _("DingTalk integration is not ready"))
        if not provider.get("is_active"):
            raise AppApiException(500, _("DingTalk integration is not enabled"))
        app_key = config.get("app_key")
        app_secret = config.get("app_secret")
        if not app_key or not app_secret:
            raise AppApiException(
                500, _("DingTalk integration is missing required credentials")
            )
    ...
    def resolve_username(identity: dict) -> str:
        ...
        if unionid:
            return str(unionid)
        raise AppApiException(500, _("Unable to resolve DingTalk user identity"))

    @classmethod
    def fetch_identity(cls, setting_type: int, code: str) -> dict:
        app_key, app_secret = cls.get_provider_credentials(setting_type)
        try:
            ...
            raise AppApiException(
                500, _("Failed to request DingTalk identity")
            ) from exc


class DingtalkLoginSerializer(DingtalkIdentitySerializer):
    def login(self):
        ...
        if user is None:
            raise AppApiException(
                500, _("DingTalk user is not bound to an existing account")
            )
