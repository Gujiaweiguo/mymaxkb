...
class DocumentView(APIView):
    ...
    @log(
        ...
        keywords: get_knowledge_document_operation_object(
...
class LarkDocumentListView(APIView):
    authentication_classes = [TokenAuth]
    ...
    def post(
        self, request: Request, workspace_id: str, knowledge_id: str, folder_token: str
    ):
        return result.success(
            DocumentSerializers.Create(
                data={"knowledge_id": knowledge_id, "workspace_id": workspace_id}
            ).lark_doc_list(folder_token, with_valid=True)
        )


class LarkDocumentImportView(APIView):
    authentication_classes = [TokenAuth]
    ...
    @log(
        menu="document",
        operate="Import lark documents",
        get_operation_object=lambda r,
        keywords: get_knowledge_document_operation_object(
            get_knowledge_operation_object(keywords.get("knowledge_id")),
            {
                "name": f"[{','.join([item.get('name') for item in r.data])}]",
                "document_list": r.data,
            },
        ),
    )
    def post(self, request: Request, workspace_id: str, knowledge_id: str):
        return result.success(
            DocumentSerializers.Create(
                data={"knowledge_id": knowledge_id, "workspace_id": workspace_id}
            ).save_lark(request.data, with_valid=True)
        )
