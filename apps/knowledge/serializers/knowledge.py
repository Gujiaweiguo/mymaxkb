...
class KnowledgeLarkCreateRequest(serializers.Serializer):
    name = serializers.CharField(required=True, label=_("knowledge name"))
    folder_id = serializers.CharField(required=True, label=_("folder id"))
    desc = serializers.CharField(
        required=False,
    ...
    app_id = serializers.CharField(required=True, label=_("app id"))
    app_secret = serializers.CharField(required=True, label=_("app secret"))
    folder_token = serializers.CharField(required=True, label=_("folder token"))


class KnowledgeLarkUpdateRequest(serializers.Serializer):
    name = serializers.CharField(required=False, label=_("knowledge name"))
    ...
    embedding_model_id = serializers.CharField(
        required=False, label=_("knowledge embedding")
    )
    app_id = serializers.CharField(required=False, label=_("app id"))
    app_secret = serializers.CharField(required=False, label=_("app secret"))
    folder_token = serializers.CharField(required=False, label=_("folder token"))
...
class KnowledgeEditRequest(serializers.Serializer):
    ...
    def get_knowledge_meta_valid_map():
        knowledge_meta_valid_map = {
            ...
            KnowledgeType.LARK: MetaSerializer.LarkMeta,
        }
...
class KnowledgeSerializer(serializers.Serializer):
    ...
    @classmethod
    def sanitize_knowledge_meta(cls, knowledge_type, meta):
        ...
        if knowledge_type == KnowledgeType.LARK:
            if meta.get("app_secret"):
                meta["app_secret"] = cls.LARK_SECRET_MASK
        return meta

    @classmethod
    def sanitize_knowledge_record(cls, record):
        if not isinstance(record, dict):
            return record
        next_record = {**record}
        next_record["meta"] = cls.sanitize_knowledge_meta(
            next_record.get("type"), next_record.get("meta")
        )
        return next_record

    @classmethod
    def sanitize_knowledge_result(cls, result):
        if isinstance(result, dict) and isinstance(result.get("records"), list):
            result["records"] = [
                cls.sanitize_knowledge_record(record)
                for record in result.get("records", [])
            ]
            return result
        if isinstance(result, list):
            return [cls.sanitize_knowledge_record(record) for record in result]
        return cls.sanitize_knowledge_record(result)

    @classmethod
    def merge_lark_meta(cls, current_meta, next_meta):
        merged_meta = dict(current_meta or {})
        next_meta = dict(next_meta or {})

        for field in ["app_id", "folder_token", "embedding_model_id"]:
            if next_meta.get(field) is not None:
                merged_meta[field] = next_meta.get(field)

        app_secret = next_meta.get("app_secret")
        if app_secret not in [None, "", cls.LARK_SECRET_MASK]:
            merged_meta["app_secret"] = app_secret
    ...
    class Query(serializers.Serializer):
        ...
        def get_query_set(self, workspace_manage, is_x_pack_ee):
            ...
            if not workspace_manage:
                query_set_dict["workspace_user_resource_permission_query_set"] = (
        ...
        def page(self, current_page: int, page_size: int):
            ...
            result = ResourceMappingSerializer().get_resource_count(result)
            return KnowledgeSerializer.sanitize_knowledge_result(result)
    ...
    class Operate(serializers.Serializer):
        ...
        def one(self):
            ...
            return {
                **knowledge_dict,
                **workflow,
                "meta": KnowledgeSerializer.sanitize_knowledge_meta(
                    knowledge_dict.get("type"), knowledge_dict.get("meta", "{}")
                ),
        ...
        def edit(self, instance: Dict, select_one=True):
            ...
            if "meta" in instance:
                if knowledge.type == KnowledgeType.LARK:
                    knowledge.meta = KnowledgeSerializer.merge_lark_meta(
    ...
    class Create(serializers.Serializer):
        ...
        def save_lark(self, instance: Dict, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                KnowledgeLarkCreateRequest(data=instance).is_valid(raise_exception=True)
            ...
            knowledge = Knowledge(
                ...
                meta={
                    ...
                    "folder_token": instance.get("folder_token"),
                    "embedding_model_id": instance.get("embedding_model_id"),
                },
            )
            knowledge.save()
            ...
            update_resource_mapping_by_knowledge(str(knowledge_id))
            return KnowledgeSerializer.sanitize_knowledge_record(
                {**KnowledgeModelSerializer(knowledge).data, "document_list": []}
            )
