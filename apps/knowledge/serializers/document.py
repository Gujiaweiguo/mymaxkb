...
class DocumentLarkListRequestSerializer(serializers.Serializer):
    pass


class DocumentLarkImportItemSerializer(serializers.Serializer):
    name = serializers.CharField(required=True, label=_("document name"))
    token = serializers.CharField(required=True, label=_("lark token"))
    type = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                ...
                message=_("Only Lark docx documents are supported for import"),
                code=500,
            )
        ],
    )
...
class DocumentSerializers(serializers.Serializer):
    ...
    class Query(serializers.Serializer):
        ...
        no_tag = serializers.BooleanField(
            required=False, default=False, allow_null=True
        )
        tag_exclude = serializers.BooleanField(
            required=False, default=False, allow_null=True
        )
        ...
        def get_query_set(self):
            ...
            if no_tag and tag_ids:
                ...
                tagged_doc_ids = QuerySet(DocumentTag).values_list(
    ...
    class Create(serializers.Serializer):
        ...
        @staticmethod
        def get_lark_knowledge(knowledge_id):
            knowledge = QuerySet(Knowledge).filter(id=knowledge_id).first()
            if knowledge is None or knowledge.type != KnowledgeType.LARK:
                raise AppApiException(
                    500, _("Knowledge type does not support lark import")
                )
            return knowledge

        @staticmethod
        def get_lark_root_token(knowledge: Knowledge):
            root_token = knowledge.meta.get("folder_token") if knowledge.meta else None
            if not root_token:
                raise AppApiException(
                    500, _("Lark root folder token is not configured")
                )
            return root_token

        @staticmethod
        def build_lark_tree(client: LarkClient, root_token: str):
            pending_folder_tokens = [root_token]
            visited_folder_tokens: Set[str] = set()
            children_by_folder: Dict[str, List[Dict]] = {}

            while pending_folder_tokens:
                current_folder_token = pending_folder_tokens.pop(0)
                if current_folder_token in visited_folder_tokens:
                    continue
                visited_folder_tokens.add(current_folder_token)
                files = client.list_folder_files(current_folder_token)
                children_by_folder[current_folder_token] = files
                for item in files:
                    if item.get("type") == "folder" and item.get("token"):
                        pending_folder_tokens.append(item.get("token"))

            return children_by_folder, visited_folder_tokens

        @staticmethod
        def get_allowed_lark_doc_map(children_by_folder: Dict[str, List[Dict]]):
            allowed_doc_map = {}
            for files in children_by_folder.values():
                for item in files:
                    token = item.get("token")
                    if token and item.get("type") == "docx":
                        allowed_doc_map[token] = item
            return allowed_doc_map

        def lark_doc_list(self, folder_token: str, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            knowledge = self.get_lark_knowledge(self.data.get("knowledge_id"))
            client = LarkClient(
                knowledge.meta.get("app_id"), knowledge.meta.get("app_secret")
            )
            root_token = self.get_lark_root_token(knowledge)
            children_by_folder, visited_folder_tokens = self.build_lark_tree(
                client, root_token
            )
            if folder_token not in visited_folder_tokens:
                raise AppApiException(
                    500, _("Folder token is outside the configured Lark root")
                )
            files = children_by_folder.get(folder_token, [])
            existing_tokens = set(
                QuerySet(Document)
                .filter(knowledge_id=knowledge.id, type=KnowledgeType.LARK)
                .values_list("meta__token", flat=True)
            )
            return {
                "files": [
                    {
                        ...
                        "token": item.get("token"),
                        "type": item.get("type"),
                        "is_exist": item.get("token") in existing_tokens,
                    }
                    for item in files
                    if item.get("type") in {"folder", "docx"}
                ]
            }

        def save_lark(self, instance_list: List[Dict], with_valid=True):
            if with_valid:
                DocumentLarkImportItemSerializer(
                    many=True, data=instance_list
            ...
            root_token = self.get_lark_root_token(knowledge)
            children_by_folder, _ = self.build_lark_tree(client, root_token)
            allowed_doc_map = self.get_allowed_lark_doc_map(children_by_folder)
            document_list = []
            pending_outcomes = []
            outcomes = []
            request_tokens = set()
                    ...
                    outcomes.append(
                        {
                            ...
                            "type": item.get("type"),
                            "status": "skipped",
                            "reason": "duplicate_request_token",
                        }
                    )
                    continue
                request_tokens.add(token)
                if item.get("token") in existing_tokens:
                    outcomes.append(
                        {
                            "name": item.get("name"),
                            "token": token,
                    ...
                    continue
                if token not in allowed_doc_map:
                    outcomes.append(
                        {
                            "name": item.get("name"),
                            "token": token,
                            "type": item.get("type"),
                            "status": "failed",
                            "reason": "token_outside_configured_root",
                        }
                    )
                    continue
                try:
                    content = client.get_document_content(token, item.get("type"))
                    document_list.append(
                        {
                            "name": item.get("name")[0:128],
                            "paragraphs": [{"content": content or item.get("name")}],
                            "meta": {
                                "token": token,
                                "type": item.get("type"),
            ...
            if document_list:
                try:
                    created_documents = DocumentSerializers.Batch(
                        data={
                            "knowledge_id": self.data.get("knowledge_id"),
                            "workspace_id": self.data.get("workspace_id"),
                        }
                    ).batch_save(document_list, with_valid=True)
                    created_document_map = {
                        document.get("meta", {}).get("token"): document
                        for document in created_documents
                    }
                    for item in pending_outcomes:
                        created_document = created_document_map.get(
                            item.get("token"), {}
                        )
                        outcomes.append(
                            {
            ...
            return {
                "document_list": created_documents,
                "items": outcomes,
                "summary": {
                    "created": len(
                        [item for item in outcomes if item.get("status") == "created"]
                    ),
                    "skipped": len(
                        [item for item in outcomes if item.get("status") == "skipped"]
                    ),
                    "failed": len(
                        [item for item in outcomes if item.get("status") == "failed"]
                    ),
                },
            }
    ...
    class DeleteDocsTag(serializers.Serializer):
        ...
        def batch_delete_docs_tag(self, instance, with_valid=True):
            ...
            valid_doc_count = Document.objects.filter(
                id__in=doc_id_list, knowledge_id=knowledge_id
            ).count()
            if valid_doc_count != len(doc_id_list):
                raise AppApiException(
                    500, _("Document id does not belong to current knowledge")
                )

            DocumentTag.objects.filter(
                document_id__in=doc_id_list, tag_id=tag_id
            ).delete()
